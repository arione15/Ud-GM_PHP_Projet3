...
8. entre la classe `animal` et la classe `image` on a mis une classe intérmédiaire `image_animal` car si on eu directement `animal (1,n) -> image(1,1)` : un animal peut avoir une ou plusieurs images et une image ne contient quun et un seul animal. Ca sera une CIF et puisqu'on a (1,1) pas (0,1), il faut mettre dans image une clé étrangère idAnimal. On sera donc bloqué si on veut faire qu'une image peut contenir plusieurs animaux. D'où la necessité, dans ce cas de recourir à une table intermédiare.