...
2. **<u>Pour n'avoir qu'une seule instance d'une classe :</u>**
   ...
   private static $monPDO = null; 
    ...
    . */
    ```
    - Créer une fonction pour pouvoir l'utiliser partout (donc static) pour, soit instancier la classe de PDO si `$monPDO` n'existe pas, soit récupérer l'instance (unique) si elle existe :
    ```php
